...
第二，生成词汇表。执行：
    ...
    python3 -m data.poetry.runner build_vocab
...
prod_pip = Pipeline(
    ...
    checkpoint_rules=CheckpointRules(
        testing=CheckpointConfig(epoch=45),
        deployment=CheckpointConfig(dirs=[resolve_saved("models/poetry_gpt")], suffix=".keras")
    )
)
